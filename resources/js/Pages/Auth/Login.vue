...
const form = useForm({
    ...
    password: "",
    remember: false,
    generalError: null, // error general (credenciales)
});
...
const validateForm = async () => {
    ...
    try {
        ...
        if (err.inner) {
            err.inner.forEach((e) => form.setError(e.path, e.message));
        }
        return false;
    }
};
...
const submit = async () => {
    ...
    form.post(route("login"), {
        onError: (errors) => {
            // Si el backend devuelve error de credenciales, lo mostramos aquí
            if (errors.email || errors.password) {
                form.generalError = "Usuario o contraseña incorrectos";
            }
        },
                        ...
                        <form @submit.prevent="submit">
                            <div>
                                <label for="email"
                                    ...
                                    electrónico</label>
                                <InputText id="email" type="email" placeholder="Correo electrónico"
                                    class="w-full md:w-[30rem] mb-2" v-model="form.email" autofocus
                                ...
                                <Password id="password" v-model="form.password" placeholder="Contraseña"
                                    :toggleMask="true" fluid class="mb-2" autocomplete="current-password"
...
.card {
    box-shadow: 1px 9px 49px -7px rgba(183, 183, 183, 0.7);
    -webkit-box-shadow: 1px 9px 49px -7px rgba(183, 183, 183, 0.7);
    -moz-box-shadow: 1px 9px 49px -7px rgba(183, 183, 183, 0.7);
}
</style>
